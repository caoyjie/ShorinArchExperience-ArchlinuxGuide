...
3. 激活快照启动项服务
   ...
   >如果你是通过 `/efi/grub/grub.cfg` 存根读取 `/boot/grub/grub.cfg` 的方式配置的 GRUB 引导，需要进行额外操作，看：[附录：Grub在btrfs文件系统的最佳配置方法](附录.md#grub-在-btrfs-文件系统的最佳配置方法)。
...
## 下一节：[安装桌面环境或窗口管理器](安装桌面环境或窗口管理器.md)
